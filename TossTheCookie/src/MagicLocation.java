...
/**
 * Created by smitatm on 12/16/15.
 */
public class MagicLocation extends Thread {
        ...
        try {
            ...
            while (remoteLogin) {
             ...
             // Get the remote server map and communicate with magic player
                    ...
                    //   while (magicResponse.startsWith("102")){
                    GameService.writeResponse(localMessageWriter, remoteResponse);
                    // }
                }
                //100 denotes the end of the map
                ...
                //login error
                else if (remoteResponse.startsWith("400")) {
                    GameService.writeResponse(localMessageWriter, remoteResponse);
    ...
    private void handleMagicResponse(PrintWriter magicMessageWriter, String magicResponse) {
        ...
        //If response start with the 102 shows successful login
        ...
        //login error
        else if (magicResponse.startsWith("400")) {
            try {
                GameService.writeResponse(magicMessageWriter, magicResponse);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        } else if (magicResponse.startsWith("500")) {
            try {
                GameService.writeResponse(magicMessageWriter, magicResponse);
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else if (magicResponse.startsWith("103")) {
            try {
                GameService.writeResponse(magicMessageWriter, magicResponse);
            } catch (IOException e) {
                e.printStackTrace();
